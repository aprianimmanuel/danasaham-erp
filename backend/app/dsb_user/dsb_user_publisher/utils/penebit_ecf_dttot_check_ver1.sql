WITH dsb_user_publisher AS (
    SELECT
        ...
        NULLIF(users.email, '') AS "users_email_registered",
        ...
        bank_account.account_number AS "publisher_bank_account_number",
        lk2.description AS "publisher_bank_account_provider_name",
        CASE
            ...
            WHEN pengurus.is_company IS NULL AND pengurus.is_contact_person = 'FALSE' AND pengurus.is_owner IS NULL THEN 'KARYAWAN'
            WHEN pengurus.is_company IS NULL AND pengurus.is_contact_person IS NULL AND pengurus.is_owner IS NULL THEN NULL
            ELSE NULL
        ...
        CASE
            WHEN pengurus.dob = 'NaN-NaN-NaN' THEN NULL
            ELSE NULLIF(TO_CHAR(TO_DATE(pengurus.dob, 'DD/MM/YYYY'), 'YYYY-MM-DD'), '')
        END AS "publisher_tgl_lahir_pengurus",
    ...
    LEFT JOIN bank_account ON publisher_finance.bank_account_bank_account_id = bank_account.bank_account_id
    LEFT JOIN lookup lk2 ON bank_account.bank_code_lookup_id = lk2.lookup_id
    WHERE
...
SELECT
    ...
    "publisher_bank_account_number",
    "publisher_bank_account_provider_name",
