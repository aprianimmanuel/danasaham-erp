...
from app.dsb_user_publisher.models import (  #type: ignore # noqa: PGH003
    dsb_user_publisher,
)
...
def save_data_to_model(df: pd.DataFrame, document: dsb_user_publisher, user: User) -> None:
    with transaction.atomic():
        for _index, row, in df.iterrows():
            # Check if a record with the same corporate_pengurus_id already exist
            existing_record = dsb_user_publisher.objects.filter(publisher_pengurus_id=row["publisher_pengurus_id"]).first()
