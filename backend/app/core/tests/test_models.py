...
from django.test import TestCase, override_settings  #type: ignore # noqa: PGH003
...
class UserModelTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.password = "Testp@ss!23"  # noqa: S105
    ...
    def test_create_user_with_email_successful(self) -> None:
        ...
        assert self.user.email == self.email  # noqa: S101
        assert self.user.check_password(self.password)  # noqa: S101
    ...
    def test_create_superuser(self) -> None:
        ...
        assert superuser.is_staff  # noqa: S101
        assert superuser.is_superuser  # noqa: S101

    def test_new_user_email_normalized(self) -> None:
        """Test email is normalized for new users."""
        email = "testuser2@EXAMPLE.COM"
        user = User.objects.create_user(email, "sample123", password="Testp@ss!23")  # noqa: S106
        assert user.email == email.lower()  # noqa: S101

    def test_new_user_without_email_raises_error(self) -> None:
        """Test that creating a user without an email raises a ValueError."""
        with pytest.raises(ValueError):  # noqa: PT011
            User.objects.create_user("", "test123", password="Testp@ss!23")  # noqa: S106

    def test_user_id_as_primary_key(self) -> None:
        """Test that the user_id field is used as the primary key."""
        assert hasattr(self.user, "user_id")  # noqa: S101
        assert self.user.user_id is not None  # noqa: S101
...
class UserProfileModelTests(TestCase):
    ...
    def test_user_profile_creation(self) -> None:
        ...
        assert self.user_profile.user.username == "testuser"  # noqa: S101
        assert self.user_profile.bio == "This is a test bio."  # noqa: S101
        assert self.user_profile.phone_number == "1234567890"  # noqa: S101
        assert self.user_profile.birth_date == date(1990, 1, 1)  # noqa: S101
        assert self.user_profile.first_name == "Test"  # noqa: S101
        assert self.user_profile.last_name == "User"  # noqa: S101

    def test_user_profile_str(self) -> None:
        """Test the string representation of the user profile."""
        assert str(self.user_profile) == "testuser"  # noqa: S101
...
class DttotDocModelTest(TestCase):
    ...
    def test_str_representation(self) -> None:
        ...
        assert str(self.dttot_doc) == "John Doe - Personal"  # noqa: S101


@override_settings(MEDIA_ROOT=os.path.join(settings.BASE_DIR, "media", "test_media"))  # noqa: PTH118
class DocumentModelTests(TestCase):
    ...
    def test_document_creation(self) -> None:
        ...
        assert document.document_name == "Test Document"  # noqa: S101
        assert document.description == "A test document description."  # noqa: S101
        assert document.document_type == "Type1"  # noqa: S101
        assert document.created_by == self.user  # noqa: S101
        assert document.updated_by == self.user  # noqa: S101
    ...
    def test_document_str(self) -> None:
        ...
        assert str(document) == "Test Document"  # noqa: S101

    def upload_document_test_helper(self, file_name, file_content, document_type) -> None:  # noqa: ANN001
        ...
        assert Document.objects.filter(pk=document.document_id).exists()  # noqa: S101
        assert document.document_file.name  # noqa: S101
    ...
    def test_document_update(self) -> None:
        ...
        assert updated_document.document_name == "Updated Name"  # noqa: S101
        assert updated_document.document_type == "Updated Type"  # noqa: S101
