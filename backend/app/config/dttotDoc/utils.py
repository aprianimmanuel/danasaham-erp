...
def handle_dttot_document(document, row_data, user_id):
    try:
        ...
        if not any(
            difflib.SequenceMatcher(None, kode_densus, existing).ratio() > 0.90
            ...
            if dttot_serializer.is_valid():
                ...
                logger.info(
                    f"Successfully saved row data for document ID {document.document_id}, dttot ID {instance.dttot_id}",
                )
        ...
        logger.exception(
            f"Error processing row data for document ID {document.document_id}: {e}",
        )
        raise
