...
@router.register_decorator(r"documents/dttotdocs/list/<uuid:document_id>/", name="dttot-doc-list")
class DttotDocListView(GenericAPIView):
    ...
    def get(self, request, document_id):
        ...
        logger.info(f"Successfully fetched {len(queryset)} DTTOT documents for document ID: {document_id}")
        return Response(serializer.data)


@router.register_decorator(r"documents/dttotdocs/details/<uuid:dttot_id>/", name="dttot-doc-detail")
class DttotDocDetailView(GenericAPIView):
    ...
    def get(self, request, dttot_id):
        ...
        logger.info(f"Successfully fetched DTTOT document details for DTTOT ID: {dttot_id}")
        return Response(serializer.data)
    ...
    def put(self, request, dttot_id):
        ...
        if serializer.is_valid():
            ...
            return Response(serializer.data)
        logger.error(f"Failed to update DTTOT document for DTTOT ID: {dttot_id}, errors: {serializer.errors}")
    ...
    def patch(self, request, dttot_id):
        ...
        if serializer.is_valid():
            serializer.save()
            logger.info(f"Successfully partially updated DTTOT document for DTTOT ID: {dttot_id}")
            return Response(serializer.data)
        logger.error(f"Failed to partially update DTTOT document for DTTOT ID: {dttot_id}, errors: {serializer.errors}")
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
